...
2.  **Phím tắt soạn thảo gia tốc (Editing Accelerations)**:
    ...
    - **Đóng buffer đang mở cực nhanh (Universal Quit)**: Nhấn **`Ctrl + q`** (hoặc gõ **`q`**) ở Normal Mode để đóng nhanh buffer/cửa sổ đang mở theo thứ tự ưu tiên tối ưu (đóng float window → Special panels → Normal file buffers → Snacks Explorer)!

...
2.  **Phím tắt soạn thảo gia tốc (Editing Accelerations)**:
    ...
    - **Thụt lề nhanh (Tab / Shift-Tab Indentation)**: Trong **Normal Mode**, nhấn **`Tab`** hoặc **`Shift + Tab`** để thụt lề dòng code hiện tại sang phải hoặc trái. Trong **Visual Mode**, nhấn **`Tab`** hoặc **`Shift + Tab`** để thụt lề nhanh cả khối code đang chọn mà vẫn **giữ nguyên vùng chọn (Visual Selection)** để bạn căn chỉnh liên tục cực kỳ nhanh chóng và tiện lợi!
    - **Đóng và lưu buffer (Save & Close Buffer)**: Nhấn **`Ctrl + q`** ở Normal Mode để tự động lưu (nếu có thay đổi) và đóng buffer hiện tại. Bạn có thể gõ nhanh phím **`q`** (Universal Quit) để đóng các buffer/cửa sổ trợ giúp theo thứ tự ưu tiên tối ưu.
    - **Đóng phân tách cửa sổ (Close Window)**: Nhấn **`Ctrl + x`** ở Normal Mode để đóng cửa sổ/phân tách màn hình (split window) hiện tại.
